...
int main(int argc,char** argv)
{
    int ierr;	
    MPI_Status status;
    int my_id, root_process, num_rows, num_procs,an_id, num_elements_to_receive, avg_elements_per_process, 
	...
	int id;
	  ...
	  //printf("start elelemnt = %d and end element  = %d\n",arr[start_row],arr[end_row]);
        ...
        */
        for( id=1;id<num_procs;id++)
        {
            ierr = MPI_Recv( &isFound, 1, MPI_INT, MPI_ANY_SOURCE,return_data_tag, MPI_COMM_WORLD, &status);
  	    //printf("isFound = %\n",isFound);
	...
	if(isFound>=0)
	{
	   printf("Element found at index from slave  = %d\n",my_id*718+isFound );
        ...
        ierr = MPI_Send( &isFound, 1, MPI_INT, root_process, return_data_tag, MPI_COMM_WORLD);
    }	
    ierr = MPI_Finalize();
	//printf("Execution time = %f seconds\n",(finish-start));
}
